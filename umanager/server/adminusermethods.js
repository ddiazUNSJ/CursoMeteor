...
// Daniel
// Este programa lo baje de https://github.com/kenyee/meteor-accounts-admin-ui
  ...
  updateUser: function(options) {
    ...
    if (user.email) {
      ...
      Meteor.users.update({ _id: options.id },
			{ $set: {'profile.name': options.username,
			    emails: [{ "address": options.email, "verified": true}], // Si le pongo false va a pedir verificacion via email
			    roles: options.roles}
			});
    }
